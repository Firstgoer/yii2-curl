 ...
 * @category  Web-yii2
 * @package   yii2-curl
 * @author    Eugen Ivanov <firstgoer.ru>
 * @copyright 2013-2015 Nils Gajsek<firstgoer.ru>
 * @license   http://opensource.org/licenses/MIT MIT Public
 * @version   1.0.0
 * @link      http://www.firstgoer.ru
 *
 */
...
class Curl
{
	 ...
	 * Also sets the $user_agent to $_SERVER['HTTP_USER_AGENT'] if it exists, 'Curl/PHP '.PHP_VERSION.'
	 * (http://github.com/firstgoer/curl)' otherwise
	 **/
	...
	function request($method, $url, $vars = array(), $enctype = null)
	{
		...
		if (YII_ENV_DEV) {
			if (!empty($startTime)) {
				$makeTime = time() - $startTime;
				Yii::info('Request to '.$url.' exceeded '.$makeTime.' sec', 'Curl');
			}
		}
		if (!$response) {
			throw new HttpException(curl_error($this->request), curl_errno($this->request));
		}
